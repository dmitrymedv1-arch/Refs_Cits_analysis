...
# Установка пути к config.toml в корневой папке
os.environ['STREAMLIT_CONFIG_FILE'] = os.path.join(os.path.dirname(__file__), 'config.toml')
...
class CitationAnalyzer:
    ...
    def save_citation_analysis_to_excel(self, citing_articles_df: pd.DataFrame, citing_details_df: pd.DataFrame,
        ...
        try:
            ...
            # Use pandas ExcelWriter instead of openpyxl directly for better compatibility
...
Total citation relationships: {len(citing_articles_df) if not citing_articles_df.empty else 0}
Total unique citing articles: {len(self.get_unique_citations(citing_articles_df)) if not citing_articles_df.empty else 0}
...
"""
                ...
                summary_df = pd.DataFrame([line for line in summary_content.split('\n') if line.strip()])
                summary_df.to_excel(writer, sheet_name='Report_Summary_Citations', index=False, header=False)

                # Prepare data for other sheets
                ...
                # Remove authors_surnames column if exists
                ...
                for sheet_name, method in analysis_methods:
                    try:
                        ...
                        st.warning(f"Could not generate {sheet_name}: {e}")
                ...
                try:
                    ...
                    title_word_df.to_excel(writer, sheet_name='Title_Word_Frequency_Citations', index=False)
                except Exception as e:
                    st.warning(f"Could not generate title word frequency: {e}")
            ...
            with open(excel_path, "rb") as file:
                btn = st.download_button(
                    ...
                    data=file,
                    file_name=f"citation_analysis_results_{timestamp}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

            shutil.rmtree(temp_dir)
            return f"citation_analysis_results_{timestamp}.xlsx"

        except Exception as e:
            st.error(f"Error saving citation analysis: {e}")
    ...
    def process_doi_sequential(self, doi_list: List[str]) -> tuple[pd.DataFrame, pd.DataFrame, int, int, List[str]]:
        ...
        # Progress bar for collecting references
        progress_bar = st.progress(0)
        status_text = st.empty()
        ...
        for i, doi in enumerate(doi_list):
            try:
                ...
                st.warning(f"Error collecting references for {doi}: {e}")
        ...
        # Progress bar for searching DOIs by title
        if titles_to_search:
            progress_bar = st.progress(0)
            status_text = st.empty()
        ...
        # Progress bar for processing unique DOIs
        progress_bar = st.progress(0)
        status_text = st.empty()
        ...
        # Progress bar for processing source articles
        progress_bar = st.progress(0)
        status_text = st.empty()
        ...
        try:
            ...
            st.warning(f"Error enhancing incomplete data: {e}")

        try:
            source_articles_df = self.enhance_incomplete_data(source_articles_df)
        except Exception as e:
            st.warning(f"Error enhancing source articles data: {e}")
...
Unique references: {len(self.get_unique_references(combined_df)) if not combined_df.empty else 0}
...
"""
                ...
                summary_df = pd.DataFrame([line for line in summary_content.split('\n') if line.strip()])
                summary_df.to_excel(writer, sheet_name='Report_Summary', index=False, header=False)
                ...
                try:
                    ...
                    title_word_df = pd.DataFrame(title_word_data)
                    title_word_df.to_excel(writer, sheet_name='Title_Word_Frequency', index=False)
                except Exception as e:
                    st.warning(f"Could not generate title word frequency: {e}")
            ...
            with open(excel_path, "rb") as file:
                btn = st.download_button(
                    label="📥 Download References Analysis Excel",
                    data=file,
                    file_name=f"references_analysis_results_{timestamp}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

            shutil.rmtree(temp_dir)
            return f"references_analysis_results_{timestamp}.xlsx"

        except Exception as e:
            st.error(f"Error saving references analysis: {e}")
    ...
    def display_analysis_results(self, combined_df: pd.DataFrame, source_articles_df: pd.DataFrame,
        ...
        try:
            st.header("📊 References Analysis Results")
            
            if combined_df.empty and source_articles_df.empty:
                st.error("No data available - generating error report")
                excel_name = self.save_all_data_to_excel(combined_df, source_articles_df, doi_list, total_references, unique_dois, all_titles)
            ...
            with col4:
                st.metric("Unique References", len(unique_df))

            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Successful References", successful_refs)
            with col2:
                st.metric("Failed References", len(combined_df[combined_df['error'].notna()]))
            with col3:
                st.metric("Processing Time", f"{stats.get('elapsed_seconds', 0):.1f}s")
            with col4:
            ...
            st.dataframe(pd.DataFrame(ref_counts))

            # Display sample data
            if not source_articles_df.empty:
                st.subheader("Source Articles (Sample)")
                display_cols = ['source_doi', 'title', 'authors_with_initials', 'year', 'journal_abbreviation', 
                              'citation_count_openalex', 'altmetric_score']
            ...
            analyses = [
                ...
                ('Top Authors', self.analyze_authors_frequency),
                ('Top Journals', self.analyze_journals_frequency),
                ('Top Affiliations', self.analyze_affiliations_frequency)
            ]
            ...
            for title, method in analyses:
                ...
                try:
                    ...
                    if not result.empty:
                        if title == 'Top Journals':
                            display_cols = ['journal_abbreviation', 'frequency_total', 'percentage_total', 'frequency_unique', 'percentage_unique']
                            if all(col in result.columns for col in display_cols):
                                st.dataframe(result[display_cols].head(10))
                            else:
            ...
            excel_name = self.save_all_data_to_excel(combined_df, source_articles_df, doi_list, total_references, unique_dois, all_titles)
            st.success(f"All data archived and ready for download as: {excel_name}")
...
def main():
    ...
    st.markdown("Analyze references and citations for scientific articles using DOI identifiers")
        ...
        if st.button("Analyze References", type="primary"):
            if doi_input_references:
                ...
                if doi_list:
                    ...
                    with st.spinner("Processing references analysis..."):
                        try:
    ...
    with tab2:
        st.header("Analyze Citing Articles")
        ...
        if st.button("Analyze Citing Articles", type="secondary"):
            if doi_input_citing:
                ...
                if doi_list:
                    ...
                    with st.spinner("Processing citing articles analysis..."):
                        try:
                            ...
                            if citing_results:
                                st.header("📊 Citing Articles Analysis Results")
                                ...
                                st.dataframe(pd.DataFrame(citation_counts))
                                ...
                                # Generate and provide download link
                                excel_name = analyzer.save_citation_analysis_to_excel(citing_articles_df, citing_details_df, doi_list, citing_results, all_citing_titles)
                                st.success(f"Complete citation analysis archived and ready for download as: {excel_name}")
                            else:
                                st.info("No citing articles found.")
