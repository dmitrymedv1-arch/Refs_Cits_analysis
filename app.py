...
class AdvancedCitationAnalysis:
    ...
    def calculate_intellectual_debt(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Исправление: Принудительно преобразуем в numeric перед sum
        df['citation_count_openalex_numeric'] = pd.to_numeric(df['citation_count_openalex'], errors='coerce').fillna(0)
        intellectual_debt = df.groupby('source_doi')['citation_count_openalex_numeric'].sum().reset_index()
    ...
    def calculate_citation_premium(self, unique_refs: pd.DataFrame) -> pd.DataFrame:
        ...
        # Исправление: Преобразование в numeric
        unique_refs_copy['citation_count_openalex_numeric'] = pd.to_numeric(unique_refs_copy['citation_count_openalex'], errors='coerce').fillna(0)
        year_baseline = unique_refs_copy.groupby('year_numeric')['citation_count_openalex_numeric'].median().to_dict()
        ...
        unique_refs_copy['citation_premium'] = unique_refs_copy.apply(
            lambda x: (x['citation_count_openalex_numeric'] / x['expected_citations'] 
    ...
    def calculate_citation_dna(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def calculate_dna_metrics(source_group):
            ...
            # Исправление: Преобразование в numeric
            source_group['citation_count_openalex_numeric'] = pd.to_numeric(source_group['citation_count_openalex'], errors='coerce').fillna(0)
            intellectual_debt = source_group['citation_count_openalex_numeric'].sum()
            
            # Средний premium
            if 'citation_premium' in source_group.columns:
                source_group['citation_premium_numeric'] = pd.to_numeric(source_group['citation_premium'], errors='coerce').fillna(0)
                premium_avg = source_group['citation_premium_numeric'].mean()
            else:
                premium_avg = 0
...
class CitationAnalyzer:
    ...
    def safe_calculate_annual_citation_rate(self, citation_count, publication_year, current_year=None):
        ...
        try:
            citation_count = pd.to_numeric(citation_count, errors='coerce')
            if pd.isna(citation_count) or citation_count == 0:
                return 0.0
    ...
    def analyze_citation_journals_frequency(self, citations_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for journal in journal_freq['journal_abbreviation']:
                ...
                # Исправление: Преобразование в numeric
                journal_articles['citation_count_crossref_numeric'] = pd.to_numeric(journal_articles['citation_count_crossref'], errors='coerce').fillna(0)
                journal_articles['citation_count_openalex_numeric'] = pd.to_numeric(journal_articles['citation_count_openalex'], errors='coerce').fillna(0)

                total_crossref_citations = journal_articles['citation_count_crossref_numeric'].sum()
                total_openalex_citations = journal_articles['citation_count_openalex_numeric'].sum()

                avg_crossref_citations = journal_articles['citation_count_crossref_numeric'].mean() if len(journal_articles) > 0 else 0
                avg_openalex_citations = journal_articles['citation_count_openalex_numeric'].mean() if len(journal_articles) > 0 else 0
    ...
    def save_citation_analysis_to_excel(self, citing_articles_df: pd.DataFrame, citing_details_df: pd.DataFrame,
        ...
        try:
            ...
            # NEW ADVANCED ANALYSIS - с try-except
            try:
            ...
            # Исправление: Безопасное суммирование
            def safe_sum(series):
                return pd.to_numeric(series, errors='coerce').sum()

            total_debt = safe_sum(intellectual_debt_df['intellectual_debt']) if not intellectual_debt_df.empty else 0
            avg_premium = citation_premium_df['citation_premium'].mean() if not citation_premium_df.empty else 0
...
Total Intellectual Debt: {total_debt:.1f}
Average Citation Premium: {avg_premium:.2f}x
            ...
            if doi not in self.unique_ref_data_cache:
                try:
                    ...
                    self.unique_ref_data_cache[doi] = {
                        ...
                        'journal_abbreviation': 'Error', 'publisher': 'Error',
                        'citation_count_crossref': 0, 'citation_count_openalex': 0,  # Изменено на 0 вместо 'N/A'
        ...
        for i, doi in enumerate(doi_list):
            try:
                ...
                source_articles.append({
                    ...
                    'publisher': 'Error', 'citation_count_crossref': 0, 'citation_count_openalex': 0,
                    'annual_citation_rate_crossref': 0, 'annual_citation_rate_openalex': 0,
                    'years_since_publication': 1, 'affiliations': 'Error', 'countries': 'Error',
                    'error': str(e)
                })
                ...
                if ref_doi and self.validate_doi(ref_doi) and ref_doi in self.unique_ref_data_cache:
                    ...
                    if found_doi and found_doi in self.unique_ref_data_cache:
                        ...
                        results.append({
                            ...
                            'citation_count_crossref': 0, 'citation_count_openalex': 0,
                            'annual_citation_rate_crossref': 0, 'annual_citation_rate_openalex': 0,
                            'years_since_publication': 1, 'affiliations': 'Unknown', 'countries': 'Unknown',
            ...
            if doi_input_references:
                ...
                if doi_list:
                    ...
                    try:
                        ...
                        # Сохранение в session_state для Advanced Analytics
                        st.session_state.combined_df = combined_references_df
                        st.session_state.source_articles_df = source_articles_df
                        st.session_state.all_titles = all_titles
                        st.session_state.doi_list = doi_list
                        st.session_state.total_references = total_references
                        st.session_state.unique_dois = unique_dois
